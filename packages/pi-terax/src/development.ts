...
const GUIDES: Record<DevelopmentCapability, Omit<DevelopmentGuide, "gotchas">> = {
  ...
  feature: {
    ...
    invariants: [
      ...
      "A side panel is mounted and unmounted by its visibility flag, never left collapsed: two collapsible siblings cannot both stay collapsed and the layout solver will re-expand one past its own maxSize.",
      "Work that must happen at a wall-clock instant is timed in Rust, not by a webview timer, which is throttled while the window is backgrounded. Keep the schedule maths pure in the frontend and hand the native side one absolute deadline.",
      "Persisted user-authored text stays out of app.snapshot; expose it through a dedicated command whose call is an explicit request.",
    ],
