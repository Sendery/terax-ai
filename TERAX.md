...
- **updater/** — auto-updater UI built on `tauri-plugin-updater`. Supports a `updateChannel` preference (`stable` | `dev`, selected in About). Stable on macOS/Windows uses the signed Tauri endpoint (latest non-prerelease); the `dev` channel and all Linux checks resolve through the GitHub releases API (`lib/releases.ts`, pure + tested) filtered by the `prerelease` flag and install manually by opening the release. Every release reference derives from `BUILD_INFO.repository` (Sendery), never a foreign remote.
