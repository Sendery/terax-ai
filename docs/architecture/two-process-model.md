...
- `pty_shell_name` - shell detection
...
- `download_release_asset` - SSRF-guarded download of a GitHub release installer to the OS downloads dir (manual/dev updates)
...
- `agent_enable_claude_hooks` / `agent_claude_hooks_status` - install/status Claude Code terminal hooks
- `agent_cli_which` / `agent_cli_spawn` / `agent_cli_kill` - locate and manage CLI coding-agent processes
...
- `open_notes_window` / `close_notes_window` - floating notes window
- `capture_persist` - persist an in-app visual capture PNG to the private app-cache dir
- `slot_monit_query` - query the user's `slot-monit` shell helper for the status-bar health bulb
- `external_command_respond` - respond to an external command-registry invocation (pi-terax bridge)
