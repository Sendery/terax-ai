...
describe("editableFields", () => {
  ...
  it("exposes title and note for link-backed cards", () => {
    const card = createCardFromUrl("https://example.com/x");
    expect(editableFields(card)).toEqual(["title", "note"]);
  });
});

describe("draftFromCard", () => {
  it("seeds body from a text card and leaves note empty", () => {
  ...
  it("seeds title and note from a link card", () => {
...
describe("buildEditPatch", () => {
  ...
  it("never blanks a text body (keeps original when draft is empty)", () => {
    ...
    const patch = buildEditPatch(card, { title: "", body: "   ", note: "" });
  ...
  it("trims whitespace around edited values", () => {
    ...
    expect(buildEditPatch(card, { title: "  T  ", body: "  new  ", note: "" }))
